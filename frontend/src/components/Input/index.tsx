...
export const PrimaryInput = (props: InputBasicProps) => { 
  ...
  const inputElements: {[key: string]: any} = {
    'textarea': <textarea rows={4} value={value.toString()} onChange={(e) => onChange(e.target.value)} />,
    'checkbox': <input type={type} checked={value.toString() === 'true' ? true : false} onChange={(e) => onChange(e.target.checked)} />,
    'other': <input type={type} value={value.toString()} onChange={e => onChange(e.target.value)} />,
  }
